//! Health computation pipeline.
//!
//! Responsible for:
//!   - `compute_health` — run every enabled check, aggregate score,
//!     apply dismissals, persist the snapshot.
//!   - `compute_single_check` — run a single check in isolation.
//!   - `compute_link_check` — async broken-internal-links check.
//!   - `apply_dismissals` — filter dismissed entries out of check data.
//!
//! The fix orchestrator lives in `run_fix.rs`. Shared types live in
//! `types.rs`, scoring in `score.rs`.

use super::score::aggregate_score;
use super::types::{HealthCheckOverride, HealthCheckResult, HealthReport, HealthStatus};
use super::{checks, custom, link_resolution, pair_key};
